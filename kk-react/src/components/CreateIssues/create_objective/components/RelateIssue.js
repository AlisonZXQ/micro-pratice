  ...
  render() {
    ...
    return (<span>
      <div className={styles.relateIssue}>
        <span style={{ marginLeft: '12px' }}>关联JIRA目标</span>
      </div>
      <div className={styles.jiraTip}>
        <span>
          设置关联JIRA目标后，对应的JIRA单子与EP的目标单将双向绑定，字段与状态将在绑定后再编辑时进行同步。
        </span>
        <span className='u-mgt5'>
          默认不配置关联JIRA目标情况下，EP将自动创建一个EP目标单的副本进行同步。
        </span>
      </div>
      <FormItem className='u-mgt10' label="当前关联的JIRA目标" {...formLayout}>
        {
