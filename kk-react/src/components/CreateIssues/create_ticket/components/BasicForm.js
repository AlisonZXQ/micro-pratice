...
class BasicForm extends Component {
  ...
  componentDidMount() {
    if (this.props.productid) {
      ...
      this.getTicketCustomList(this.props.productid);
    }
  }
  ...
  componentWillReceiveProps(nextProps) {
    if (this.props.productid !== nextProps.productid) {
      ...
      this.getTicketCustomList(nextProps.productid);
    }
  }
  ...
  getModuleList = (id) => {
    getModuleList(id, (result) => this.setState({ moduleList: result, loading: false }));
  }
  ...
  handleChangeModule = (value, data) => {
    const currentData = data.props.data;
    const responseUser = currentData.ticketResponseUser || {};
    const requireUser = currentData.ticketRequireUser || {};
    if (responseUser && responseUser.id) {
      this.props.form.setFieldsValue({
        responseemail: responseUser.email,
      });
  ...
  getDefaultSubProduct = () => {
    const { ticketDetail, type } = this.props;
    ...
    if (ticketDetail && ticketDetail.subproduct) {
      return ticketDetail.subproduct.id;
    }
  }
  ...
  render() {
    const { currentUser, ticketDetail: detail, productList, showProduct, productid } = this.props;
    ...
    return (
      <Spin spinning={this.state.loading}>
          ...
          <Col span={12}>
            <FormItem label="模块" {...formLayout}>
              {getFieldDecorator('moduleid', {
                initialValue: ticketDetail && ticketDetail.productModule && ticketDetail.productModule.id,
              })(
                <Select
                  style={{ width: '100%' }}
                  placeholder='请选择模块'
          ...
          <Col span={12}>
            <FormItem label="优先级" {...formLayout}>
              {getFieldDecorator('level', {
                ...
                initialValue: ticket.level || LEVER_MAP.P1,
              })(
          ...
          <Col span={12}>
            <FormItem label="期望上线时间" {...formLayout}>
              {getFieldDecorator('expect_releasetime', {
                initialValue: ticket.expect_releasetime ? moment(ticket.expect_releasetime) : undefined,
              })(
        ...
        <CustomField
          ...
          type="ticket"
          valueMap={valueMap}
          create={!Object.keys(ticketDetail).length}
        />
...

        {
          ...
          <FormItem label="描述" {...bigFormLayout}>
            {getFieldDecorator(`custom-${getSystemDescription(customList).id}-${getSystemDescription(customList).type}`, {
              initialValue: getSystemDescriptionEditValue(ticketCustomFieldRelationInfoList) || getSystemDescription(customList).defaultValue,
            })(
