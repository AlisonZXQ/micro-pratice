...
class index extends Component {
  ...
  getAdviseHistory = () => {
    const { adviseHistory } = this.props;
    const arr = [];
    adviseHistory.forEach(it => {
      const adviseObj = it.adviseHistory || {};
      const optUser = it.optUser || {};

      let changelog =[];
      if( adviseObj.changelog ){
        try{
          changelog = JSON.parse(adviseObj.changelog);
        }catch(e){
          changelog = [];
        }
      }
      ...
      if(this.isRejectState(changelog)){
        ...
        arr.unshift({
          updatetime: adviseObj.updatetime,
  ...
  render() {
    ...
    return ([
      <Table
        ...
        dataSource={this.getAdviseHistory()}
        pagination={false}
