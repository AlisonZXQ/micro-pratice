...
class LookDesign extends Component {
  state = {
    visionList: [],
  }
  ...
  getDefaultData = (editData) => {
    if (editData && editData.visionUser5) {
      this.setState({ visionList: [editData.visionUser5] });
    }
  }
  ...
  handleSelect = (value, data) => {
    ...
    setFieldsValue({ visionUser5: data.props.data });
  }
  ...
  handleChangeSlider = (value) => {
    const { form: { setFieldsValue } } = this.props;
    setFieldsValue({ pageNum5: value });
  }

  handleChangePage = (value) => {
    const { form: { setFieldsValue } } = this.props;
    if (value >= 100) {
      setFieldsValue({ slider5: 100 });
    } else {
      setFieldsValue({ slider5: value || 0});
    }
  }

  getSilderNum = () => {
    const { data } = this.props;
    if (data && data.pageNum5) {
      if (data.pageNum5 >= 100) {
        return 100;
      } else {
        return data.pageNum5;
      }
    } else {
      return 1;
    }
  ...
  render() {
    ...
    return (<span>
      <Card className={styles.cardStyle}>
        ...
        <FormItem>
          {
            getFieldDecorator('designDemand5', {
              initialValue: data && data.designDemand5,
      ...
      <Card className={styles.cardStyle}>
        ...
        <FormItem>
          <Row>
            <Col span={4}>
              {
                getFieldDecorator('pageNum5', {
                  initialValue: data && data.pageNum5 ? data.pageNum5 : 1,
                  rules: [
      ...
      <Card className={styles.cardStyle}>
        ...
        <FormItem>
          ...
          {
            importantTitle[getFieldValue('grade5')] &&
            <div style={{ border: '1px solid #E1E3E6', background: '#F4F4F4', paddingBottom: '10px' }}>
              <div className="f-fwb" style={{ color: '#333333' }}>{importantTitle[getFieldValue('grade5')]}</div>
              {
                importantRate[getFieldValue('grade5')] && importantRate[getFieldValue('grade5')].map(it =>
      ...
      <Card className={styles.cardStyle}>
        ...
        <FormItem>
          <Row>
            ...
            <Col span={22}>
              {
                getFieldDecorator('visionId5', {
                  initialValue: data && data.visionUser5 && data.visionUser5.email,
                  rules: [{ required: true, message: '此项必填！' }]
                })(
                  <Select
                    allowClear
              ...
              }
              {
                ...
                <div>
                  ...
                  {local.visionUser5.map((it, index) =>
              ...
              {
                getFieldDecorator('visionUser5', {
                  initialValue: data && data.visionUser5,
                })(
      ...
      <Card className={styles.cardStyleRemark}>
        ...
        <FormItem>
          {
            getFieldDecorator('remark5', {
              initialValue: data && data.remark5,
            })(
