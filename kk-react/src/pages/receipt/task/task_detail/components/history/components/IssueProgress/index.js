...
class index extends Component {
  ...
  customDot = (dot, { status, index }) => {
    ...
    const state = data && data.task && data.task.state;
    let current = bugTaskNameArr.findIndex(it => it.key === state);
    if (state === TASK_STATUS_MAP.REOPEN) {
      current = 0;
    }

    if (state === TASK_STATUS_MAP.REOPEN && index === 5) {
      // 已驳回的直接是蓝色
      ...
      <Step
        ...
        description={
          <p className="u-mgt10">
            {/* 已驳回时展示 */}
            {
              state === TASK_STATUS_MAP.REOPEN && !!Object.keys(obj).length && index === 5 &&
              des()
            }
            {/* 其他情况需要根据current判断 */}
            {
