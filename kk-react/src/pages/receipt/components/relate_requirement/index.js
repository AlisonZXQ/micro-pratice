...
class index extends Component {
      ...
      {rtotalData && rtotalData.map((item) => {
        return <div>
          <Link
            ...
            to={`/my_workbench/requirementdetail/Feature-${item.id}`}>{item.name}</Link>
        </div>;
      })}
    </span>;
  }
  ...
  getRtotalData = (id) => {
    ...
    getRelationLink(params).then((res) => { //关联需求数据
      if (res.code !== 200) { return message.error(res.msg) }
      this.setState({ rtotalData: res.result.requirementSet });
    }).catch((err) => {
