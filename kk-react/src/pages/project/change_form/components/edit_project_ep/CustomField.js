...
class CustomField extends Component {
  ...
  getDefaultCategory = (cascadeList) => {
    ...
    customFileds.forEach(it => {
      if (it.customField.type === 2 && it.value) {
  ...
  getFilterItems = (customData) => {
    let data = [];
    customData.filter(it => it.customField.state === 1) // 过滤掉隐藏的字段
      ...
      .forEach(it => {
        let index = -1;
        index = data.findIndex(item => item && it.customField &&
          (item.sortvalue < it.customField.sortvalue));
        if (it.customField.sortvalue === 0) {
          data.push(it); // 为0的都放在最后
        } else if (index === -1) { // 找不到比该值小的放在最前面
          data.unshift(it);
      ...
      <FormItem label={getLabel(cascadeField.fieldname)} {...formLayout}>
        {
          getFieldDecorator(`cascadeField-${cascadeField.id}`, {
            ...
            rules: [{ required: cascadeField.required === 1, message: '此项必填！' }],
          })(
            <Cascader
            ...
            return (<span>
              <FormItem label={getLabel(it.customField.name)} {...formLayout}>
                {getFieldDecorator(`custom-${it.customFieldId}`, {
                  ...
                  rules: it.customField.required === 1 && [{ required: true, message: '此项不能为空' }]
                })(
                  it.customField.type === 1 ? it.customField.inputui === 1 ?
                    <Input placeholder="请输入名称" maxLength={200} /> :
                    <TextArea placeholder="请输入名称" maxLength={200} style={{ height: '80px' }} /> :
                    <Select 
                      onSelect={(value) => this.handleSelect(value, it)} 
                      allowClear
