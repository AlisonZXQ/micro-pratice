...
import CascadeField from './components/CascadeField';
...
class ApprovalHeader extends Component {
  ...
  render() {
    ...
    const { id } = this.props.location.query;
    // 级联字段
    const diffCascadeField = list.diffCascadeField || [];
    const data = list || {};
    ...
    data.diffOther && data.diffOther.forEach(it => {
      ...
      if (changeData[it.field]) {
        if (it.field === 'priority') {
          changeData[`${it.field}-oldvalue`] = it.oldvalue && Number(it.oldvalue) !== 0 ? priorityMap[Number(it.oldvalue)] : '-';
        } else {
          changeData[`${it.field}-oldvalue`] = it.oldvalue;
        }
      } else {
    ...
    const customObj = (data.diffOther && data.diffOther.find(it => it.field === 'createCustomFields')) || {};
    const customOld = customObj.oldvalue ? JSON.parse(customObj.oldvalue) : [];
    ...
    const getOldValue = (field) => {
      return changeData[`${field}-oldvalue`];
    };
    ...
    return ([<Card className="btn98">
      <Col span={18}>
        ...
        <Row>
          <HeaderText data={list} title={title} />
        </Row>
        ...
        {/* 自定义字段 */}
        {
          ...
          <Row>
            <Col offset={1} span={23}>
              ...
              <CustomField createCustomFileds={createCustomFileds} customOld={customOld} />

              <CascadeField data={diffCascadeField} />
        ...
        <div style={{ position: 'absolute', right: '0px', width: 350 }}>
          ...
          <Row className="u-mgt40">
            ...
            <Col span={6}>
              ...
              <CommonDiffText
                newvalue={<span style={getBackGroundColor('owner')} className="u-mgt5 f-fs4 f-fr f-clb f-wwb">{this.isEmptyData(projectDetail.owner)}</span>}
                oldvalue={getOldValue('owner')}
              />
            </Col>
            <Col span={6}>
              <div className="f-fs2 u-thirdtitle f-fr f-clb">优先级</div>
              <CommonDiffText
                newvalue={<span style={getBackGroundColor('priority')} className="u-mgt5 f-fs4 f-fr f-clb">{projectDetail.priority ? priorityMap[projectDetail.priority] : '-'}</span>}
                oldvalue={getOldValue('priority')}
              />
            </Col>
          </Row>
        </div>
