...
import { Input, Row, Col, Popconfirm, Checkbox, message } from 'antd';
import uuid from 'uuid';
import { getCustomFieldSet } from '@services/system_manage';
import { deepCopy, equalsObj } from '@utils/helper';
import { CUSTOM_SELECTTYPE_MAP } from '@shared/ReceiptConfig';
...
class AddCascaderDialog extends Component {
  ...
  componentDidMount() {
    ...
    if (record && record.id && record.type === CUSTOM_SELECTTYPE_MAP.CASCADER) {
  ...
  componentWillReceiveProps(nextProps) {
    if (!equalsObj(this.props.record, nextProps.record)) {
      ...
      if (type === CUSTOM_SELECTTYPE_MAP.CASCADER) {
    ...
    >
      {
        parent.type === 'input' ?
          ...
          :
          <div
            style={{ height: '30px', background: parent.active ? styles.activeColor : '' }}
